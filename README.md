...
│   │   ├── interfaces/             #   接口层（49 个 Controller、DTO）
│   │   │   ├── auth/               #     认证接口
│   │   │   ├── user/               #     用户端接口
│   │   │   ├── merchant/           #     商家端接口
│   │   │   ├── system/             #     系统管理接口
...
│   ├── init_products_full.sql      # 商品初始数据（含图片记录）
│   └── init_other_data.sql         # 其他数据（活动、秒杀、优惠券等）
├── docker/
│   └── nginx/
...
├── start_backend.bat               # Windows 本地启动后端脚本
└── README.md                       # 本文件
```
...
```
┌─────────────────────────────────────┐
│  interfaces（接口层）                │  49 个 Controller + DTO + 参数校验
├─────────────────────────────────────┤
...
# 产物：dist/ 目录（含 WEB-INF/web.xml，支持 Tomcat 部署）
```
...
2. **选品结算** — 购物车支持多选/全选，只结算选中的商品；立即购买只结算当前商品
3. **完整的订单状态机** — CREATED -> PAID -> SHIPPED -> IN_TRANSIT -> DELIVERED -> COMPLETED
4. **模拟物流追踪** — 定时调度自动推进物流状态，生成物流轨迹，支持实时查询
5. **灵活缓存架构** — 工厂模式支持 Redis / 数据库双缓存实现，通过配置一键切换
6. **RBAC 权限模型** — 基于角色的细粒度权限控制（@PreAuthorize 注解）
7. **游戏化营销** — 优惠券发放/领取、积分签到/兑换、秒杀抢购、专题活动、排行榜
8. **安全防护** — DOMPurify XSS 防护、JWT Token 认证、密码 BCrypt 加密（各用户独立哈希）
9. **前后端分离** — 3 套前端独立部署（Vite 构建 + Tomcat 子目录），统一后端 API 服务
10. **统一异常处理** — GlobalExceptionHandler 全局捕获，标准化错误响应
11. **Tomcat SPA 支持** — 每个前端 dist 含 WEB-INF/web.xml，支持 Vue Router history 模式刷新不 404
12. **SQL 聚合优化** — Dashboard 统计使用 SQL 聚合查询，避免全表加载到内存
